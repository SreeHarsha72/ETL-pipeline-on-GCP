...
with open('employee_data.csv', mode='w', newline='') as file:
    ...
    for _ in range(num_employees):
        writer.writerow({
            ...
            "job_title": fake.job().split(' ')[0].replace(',', ''),
            "department": fake.job().split(' ')[0].replace(',', ''),  # Generate department-like data using the job() method
            "email": fake.email(),
            "address": fake.city(),
